...
export const planModeContent: SectionContent = {
  ...
  intro:
    '规划模式（Plan Mode）是 Claude Code 内置的一种权限模式：在该模式下 Claude 只能读文件、跑只读命令探索代码库，不能编辑源码或执行写入操作。分析完成后，Claude 会给出一份变更计划并停下来等你明确批准——可以选择自动执行、逐条审核编辑，或者带着反馈让它继续改计划，也可以不批准直接退出。它把"先说清楚要怎么做，再动手"变成一条硬约束，而不是靠提示词里一句"先别改"碰运气：你可以用它来确认 Claude 的方案是否贴合你自己已经想好的做法，再放行它去执行，而不是让它替你决定要做什么。',
  install: [
    {
